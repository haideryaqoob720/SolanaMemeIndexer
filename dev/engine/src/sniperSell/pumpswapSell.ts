...
export async function pumpSwapSell(mint:string, priceInSol:number, priceInUsd:number, tokenMap:RedisCache, cache:pumpSwapStore){
    let retries = 0;
    ...
    await Promise.all(
        orders.map(async(order, index) => {
            ...
            // while(retry && retryCount < MAX_RETRIES) {
                ...
                let quote:any;
                try {
                quote = await pumpSwapQuote(existingPump.marketAccount, order.tokenBuyAmount.actual ?? order.tokenBuyAmount.estimated ?? 100000000, false)
            ...
            order.sellAmount.usd.estimated = quote.amountOut * solPrice;
            ...
            //     usdSell: sellRes.outAmount * solPrice,
            // })
            order.tokenSellAmount.estimated = order.tokenBuyAmount.actual ?? order.tokenBuyAmount.estimated ?? 100000000;
            order.sellAmount = {
                sol: {
            ...
            if(order.buyAmount.sol.estimated) {
                ...
                order.status = order.buyAmount.sol.estimated < (order.sellAmount.sol.estimated ?? quote.amountOut) ? "profit" : "loss";
            }
            console.log(order)
        ...
        if (retryCount >= MAX_RETRIES) {
            console.log(`Maximum retry attempts (${MAX_RETRIES}) reached for ${mint}`);
        }
    })
);
}
