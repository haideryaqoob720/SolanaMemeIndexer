import { HttpProvider, Config, GetJupiterQuotesRequest, GetRaydiumQuotesRequest } from "@bloxroute/solana-trader-client-ts"
...
export class BloXRouteWrapper {
  ...
  private bloxUrl:string = 'https://ny.solana.dex.blxrbdn.com/api/v2/raydium/quotes'
  private bloxSwapUrl:string = 'https://ny.solana.dex.blxrbdn.com/api/v2/raydium/swap'
  private bloxJupiterUrl:string = "https://ny.solana.dex.blxrbdn.com/api/v2/jupiter/quotes"
  private bloxSwapJupiterUrl:string = "https://ny.solana.dex.blxrbdn.com/api/v2/jupiter/swap"
  ...
  constructor(authHeader:string) {
    ...
    this.client = new HttpProvider(configuration.authHeader, configuration.publicKey, configuration.privateKey);
  ...
  public async getQuote(mint:string, inAmount:number, isBuy:boolean, dex:"Raydium" | "Jupiter"){
      try {
    if(dex === "Raydium"){
...
public async getQuoteApi(
  ...
  const fetchQuote = async (): Promise<quoteResponse | null> => {
    try {
      ...
      // const blox = await axios.get(this.bloxUrl, {
      ...
      // const inputAmount = (isBuy ? inAmount : blox.data.routes[0].outAmount* Math.pow(10, decimals))*LAMPORTS_PER_SOL;
      // const outputAmount = (isBuy ? blox.data.routes[0].outAmount * Math.pow(10, decimals) : inAmount* Math.pow(10, decimals))
      const inputAmount = inAmount * Math.pow(10, decimals);
      const outputAmount = blox.data.routes[0].outAmount*LAMPORTS_PER_SOL
      // console.log({isBuy:isBuy},inputAmount, outputAmount, decimals, "calculate price blox")
      const res: quoteResponse = {
        ...
        priceSol: blox.data.routes[0].outAmount/inAmount
      };
      return res;
    } catch (error: any) {
  ...
  while (retries <= maxRetries) {
    ...
    if (retries < maxRetries) {
      ...
      console.log(`Quote fetch failed, retrying after ${currentDelay}ms...`);
...
public async swapApi(mint:string, publicKey:string, privateKey:string, inAmount:number, isBuy:boolean):
Promise<{
  transaction:any,
  outAmount:number,
  outAmountMin:number
  ...
  try {
    ...
    // const response = await axios.post(this.bloxSwapUrl, {
    ...
    console.log(inAmount, mint)
    console.error("Error during swap API call:", error.message);
    return null;
  }
}
