...
import getPoolTxStore, { PoolTxStore } from "../redis/poolTx";
...
process.on("message", async () => {
    ...
    await addTxs(tokenMap, poolTxCache);
    // await tokenMap.disconnect()
    await prisma.$disconnect();
    process.exit();
});
...
export async function syncPoolTx(poolTxCache: PoolTxStore) {
    let tx: any[] = [];
    tx = await poolTxCache.getAllTx();
    console.log("tx in cache before sync: ", tx.length);
    // tx = tx.filter((tx) => tx.signature);
    console.log("tx in cache before sync filtered out signature: ", tx.length);
    const multipleTx = await prisma.poolTransactions
        .createMany({
            data: tx,
        })
        .then(async () => {
            await poolTxCache.deleteAllTx(tx);
        })
        .catch(async (error: any) => {
            console.log(
                "error creating many tx",
                error.message,
                "creating many tx error",
            );
            await poolTxCache.deleteAllTx(tx);
        });
}
...
async function syncCache(tokenMap: RedisCache) {
    ...
    tx = tx.filter((tx) => tx.signature);
    console.log("tx in cache before sync filtered out signature: ", tx.length);
    const multipleTx = await prisma.transactions
        .createMany({
            data: tx,
        })
        .then(async () => {
            await Promise.all(
                tx.map(async (record: any, index: number) => {
                    await tokenMap.deleteFromTx(record.signature);
                }),
            )
                .catch((error: any) => {
