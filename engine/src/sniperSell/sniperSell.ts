...
export async function sniperSellRaydium(mint:string, priceInSol:number, priceInUsd:number, pnl:boolean, tokenMap:RedisCache){
    let retries = 0;
    ...
    orders.map(async(order, index) => {
        ...
        // while(retry && retryCount < MAX_RETRIES) {
            ...
            order.sellAmount.usd.estimated = parseFloat(quote.outAmount) * solPrice;
            ...
            if(order.buyAmount.sol.actual) {
                ...
                order.status = order.buyAmount.sol.actual > sellRes.outAmount ? "profit" : "loss";
            }
            console.log(order)
...
process.on("message", async () => {
  ...
  const pumpSwapCache = await getPumpSwapStore()
    const spawnProcess:boolean = await tokenMap.canSpawnNewSniperProcess(mint, orderId)
    if(!spawnProcess){
    ...
    if(dex==="PumpSwap"){
        ...
        await pumpSwapSell(mint, parseFloat(priceInSol), parseFloat(priceInUsd), tokenMap, pumpSwapCache)
    }else{
